...
//    $ps=mysqli_prepare($con,"select id, nome, endereco, telefone from aluno limit ?,?"); // prepara e envia comando ao banco de dados. O parâmetros (?) indicam, respectivamente, o primeiro registo a ser recuperado e a quantidade de registros.
//    mysqli_stmt_bind_param($ps,"ii",$inicioPagina,$tamanhoPagina);
//    mysqli_stmt_execute($ps); // executa comando SELECT
//    mysqli_stmt_bind_result($ps,$id,$nm,$ed,$tel); // associa variáveis às colunas resultantes do SELECT.
//    while(mysqli_stmt_fetch($ps)) // obtém 1 linha do resultado do SELECT, retorna FALSE quando não existirem mais linhas.
//    {
//      array_push($dados,array($id,$nm,$ed,$tel)); // inclui linha no vetor, que será usado em indexGet.php
//    }
